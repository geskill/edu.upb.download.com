...
public class ProductVersionTable {
	...
	public void updateProductVersion(int id_p, int vid, String version_name, String version_alterations,
			Date version_date, String version_identifier, String operating_systems, String additional_requirements,
			String download_size, String download_name, String download_link, int downloads_total,
			int downloads_last_week, String license_model, String license_limitations, String license_cost)
					throws SQLException {
		Statement stmt = null;
		try {
			...
			if (!stmt.execute("SELECT vid FROM download_com_product_versions " + "WHERE id_p = '" + id_p
					+ "' AND vid = '" + vid + "'")) {
				stmt.executeUpdate(
						"INSERT INTO `download_com_product_versions`(`id_p`, `vid`, `version_name`, `version_alterations`, `version_date`, `version_identifier`, `operating_systems`, `additional_requirements`, `download_size`, `download_name`, `download_link`, `downloads_total`, `downloads_last_week`, `license_model`, `license_limitations`, `license_cost`) "
								+ "VALUES('" + id_p + "', '" + vid + "', '" + version_name + "', '"
								+ version_alterations + "', '" + version_date + "', '" + version_identifier + "', '"
								+ operating_systems + "', '" + additional_requirements + "', '" + download_size + "', '"
								+ download_name + "', '" + download_link + "', '" + downloads_total + "', '"
								+ downloads_last_week + "', '" + license_model + "', '" + license_limitations + "', '"
								+ license_cost + "')");
			} else {
				stmt.executeUpdate("UPDATE `download_com_product_versions` SET `version_name`='" + version_name
						+ "',`version_alterations`='" + version_alterations + "',`version_date`='" + version_date
